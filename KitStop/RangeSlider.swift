...
class RangeSlider: UIControl {
    ...
    override func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        if lowerThumbLayer.highlighted {
            lowerValue = boundValue(lowerValue + deltaValue, toLowerValue: minimumValue, upperValue: upperValue - 2 * gapBetweenThumbs)
        } else if upperThumbLayer.highlighted {
            upperValue = boundValue(upperValue + deltaValue, toLowerValue: lowerValue + 2 * gapBetweenThumbs, upperValue: maximumValue)
        }

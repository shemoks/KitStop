...
class DataManager: NSObject{
    ...
    func authenticateWithFacebook(fbToken: String) {
        let _ = manager.apiRequest(.fbAuthenticate(), parameters: ["fb_token" : "\(fbToken)" as AnyObject, "clients" : "111" as AnyObject], headers: nil).apiResponse(completionHandler: {
            response in
            switch response.result{
            case .success(let json):
                print(json)
